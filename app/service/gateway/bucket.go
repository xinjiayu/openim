...
func buildBuckets() {
	bucketNum := g.Config().GetInt("websocketServer.bucket.Num")
	TM = &TowerManager{
		bucket:      make([]*Bucket, bucketNum),
		centralChan: make(chan *socket.SendMessage, g.Config().GetInt("websocketServer.bucket.CentralChanCount")),
	}
...
func newBucket() *Bucket {
	b := &Bucket{
		...
		BuffChan:       make(chan *socket.SendMessage, g.Config().GetInt("websocketServer.bucket.BuffChanCount")),
	}

	ConsumerNum := g.Config().GetInt("websocketServer.bucket.ConsumerNum")
	if ConsumerNum == 0 {
		ConsumerNum = 1
	}
