...
func Init() {
	...
	IdWorker, _ = snowFlake.NewWorker(ClusterId)
...
func buildNewTower(ws *websocket.Conn, clientId string) *FireTower {
	...
	t.readIn = make(chan *FireInfo, g.Config().GetInt("websocketServer.chanLens"))
	t.sendOut = make(chan *socket.SendMessage, g.Config().GetInt("websocketServer.chanLens"))
...
func (t *FireTower) sendLoop() {
	heartTicker := time.NewTicker(time.Duration(g.Config().GetInt("websocketServer.heartbeat")) * time.Second)
	for {
		select {
